...
- 从 1.2.1 及更早版本升级上来时，设置文档里存着旧的内置默认模型
  （`deepseek-official` / `deepseek-v4-flash-vision-exp` / `high`），
  1.3.0 会把它当成用户的显式覆盖，于是「跟随 DSH」不生效。
  现在加载设置时会识别这组旧值并清空，交回给路由解析。
  只有三项都等于旧默认值时才清空，用户自己填过的不受影响。

### 测试

- `dev/test-logic.cjs` 增加到 45 项，新增升级迁移相关断言。
