...
模型设置改为跟随 DSH，不再由插件另存一份。

### 变更

- `provider` 与 `model` 默认留空。留空时插件读取 `$DSH_HOME/settings.yaml` 里的
  `agent-default-model` 分节，使用 DSH 自己设置的默认模型。
  在 DSH 界面里改了默认模型，插件启动运行时就会跟着换。
- 优先级为：插件设置里两者都填 > DSH 设置文档 > 插件内置兜底值。
  provider 与 model 需要成对填写，只填一个不算覆盖，设置页会给出提示。
- `reasoning effort` 留空时同样跟随 DSH。
- 设置页「连接与模型路由」顶部显示当前生效的组合与来源（插件设置 / 跟随 DSH / 内置兜底），
  并新增「重新读取 DSH 设置」按钮。
- 环境检测新增两项：dsh 版本（执行 `--version`），以及构建产物是否落后于源码。
- 诊断区新增 DSH 版本、模型来源、构建产物状态三行。
- 自动探测的路径增加了 npm 全局安装形态（`@deepseek-ai/dsh/lib/bin.js`）。

### 修复

- 此前插件把 provider / model 复制在自己的设置里，DSH 更新换了默认模型（或改了模型名）之后
  插件不会跟着变，握手会失败。现在改成读 DSH 的设置文档，不会再出现这种不一致。

### 说明

- 这是针对「插件跑的是构建产物」这一点：`apps/cli/lib/bin.js` 是 `pnpm run build` 的产物，
  只 `git pull` 不重新构建的话，插件会继续跑旧代码而且不会报错。环境检测里新增的那一项就是查这个。
- 设置文档的读取是针对性实现的（只认「顶层分节 + 一层标量字段」），不是通用 YAML 解析器，
  目的是保持零运行时依赖。

### 测试

- `dev/test-logic.cjs` 增加到 43 项，新增：设置文档解析（引号、注释、流式写法、JSON 形态、
  缺字段与缺文件）、路由优先级、构建落后检测。
- 新增 `dev/probe-route.cjs`，用于查看当前生效的模型路由及其来源。
