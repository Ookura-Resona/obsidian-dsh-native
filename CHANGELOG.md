...
- **中英双语界面**：文案跟随 Obsidian 的界面语言（非中文系统自动用英文），
  也可以在设置里强制指定中文或英文，改完立即生效。
  错误卡片、环境检测结果、诊断区、命令名与通知都会一起切换。
- 设置页新增「界面」分组与「界面语言」选项（跟随 Obsidian / 中文 / English）。

### 变更

- 自己抛出的错误改为带稳定 `code`（`DSH_NO_CLI`、`DSH_TIMEOUT`、`DSH_RUNTIME_EXITED` 等），
  错误识别不再依赖界面语言；服务端返回的英文错误仍按正则匹配。
- 选区引用行的措辞也随语言切换（`[选中片段] 文件：…` / `[selected passage] file: …`）。

### 测试

- `dev/test-logic.cjs` 从 22 项扩到 **30 项**，新增 i18n 断言：
  中英词典 key 集合完全一致、源码里用到的每个 `t()` 词条都存在、没有死词条、占位符一一对应。
